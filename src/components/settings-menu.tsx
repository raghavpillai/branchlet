...
type SettingsStep = 'menu' | 'edit-copy-patterns' | 'edit-ignore-patterns' | 
                   'edit-path-template' | 'edit-post-cmd' | 'edit-terminal-cmd' | 
...
export function SettingsMenu({ 
  ...
  switch (step) {
      ...
      return (
        <Box flexDirection="column">
          ...
          <Box flexDirection="column" marginLeft={2} marginY={1}>
            {config.worktreeCopyIgnores.map((pattern, index) => (
              <Text key={index} color={COLORS.MUTED}>• {pattern}</Text>
            ))}
          </Box>
          <InputPrompt
    ...
    case 'edit-path-template':
      return (
        <Box flexDirection="column">
          <Text color={COLORS.INFO}>Available variables: $BASE_PATH, $WORKTREE_PATH, $BRANCH_NAME</Text>
          <InputPrompt
    ...
    case 'edit-post-cmd':
      return (
        <Box flexDirection="column">
          <Text color={COLORS.INFO}>Available variables: $WORKTREE_PATH, $BRANCH_NAME, $SOURCE_BRANCH</Text>
          <InputPrompt
            label="Post-Create Command:"
