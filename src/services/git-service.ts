import type { GitWorktree, GitBranch, GitRepository, WorktreeCreateOptions, WorktreeDeleteOptions } from '../types/index.js';
import { executeGitCommand, isGitRepository, getCurrentBranch, getDefaultBranch, handleGitError } from '../utils/index.js';
...
export class GitService {
  ...
  async listWorktrees(): Promise<GitWorktree[]> {
    ...
    for (const line of lines) {
      if (line.startsWith('worktree ')) {
        ...
        currentWorktree.branch = line.substring(23); // Remove 'branch refs/heads/'
      } else if (line === 'bare') {
    ...
    // Mark the first worktree as main if it's the git root
  ...
  async branchExists(branchName: string): Promise<boolean> {
    const result = await executeGitCommand(['show-ref', '--verify', `refs/heads/${branchName}`], this.gitRoot);
