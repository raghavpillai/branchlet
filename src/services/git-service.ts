...
export class GitService {
  ...
  async getRepositoryInfo(): Promise<GitRepository> {
    const [currentBranch, defaultBranch, worktrees, branches] =
      await Promise.all([
  ...
  async listWorktrees(): Promise<GitWorktree[]> {
    ...
    if (worktrees.length > 0) {
      ...
      if (
        firstWorktree.path === gitRoot ||
        firstWorktree.path === gitRoot.replace(/\/$/, "")
      ) {
  ...
  async listBranches(): Promise<GitBranch[]> {
    ...
    const branchOrder = new Map(
      recentBranches.map((name, index) => [name, index])
    );
